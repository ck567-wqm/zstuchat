...
export default {
  ...
  methods: {
    ...
    //获取留言信息
    async getComments() {
      const { data: res } = await this.$http.get("dynamic/selectAllDynamic");
      console.log(res);
      this.avatar = "localhost:8081/default_avatar.jpg";
    ...
    async postDynamic() {
      ...
      const { data: res } = await this.$http.post("dynamic/addNewDynamic", {
    ...
    async postComment() {
      ...
      const time = await new Date();
      await time.setHours(time.getHours() + 8);
      const { data: res } = await this.$http.post("dynamic/addNewComment", {
        uid: this.userInfo.id,
